...
\item
  ...
  secara otomatis menggunakan AI untuk menganalisis kekuatan dan area
  pengembangan dari karya saya sendiri.
\item
...
Secara keseluruhan, portofolio tugas UTS ini menunjukkan kualitas narasi
personal yang sangat kuat dan konsisten. Saya berhasil membangun sebuah
cerita yang koheren tentang identitas diri, dimulai dari akar keluarga
(UTS-1), diekspresikan melalui media lagu (UTS-2), diilustrasikan dengan
kisah inspiratif (UTS-3), hingga dirangkum dalam sebuah peta
pengembangan diri (UTS-4). Tema utama yang diusung---pentingnya empati,
kepedulian, dan pemberdayaan orang lain---terjalin dengan sangat baik di
setiap tugas.

\begin{itemize}
\tightlist
\item
  \textbf{UTS-1 (All About Me):} Narasi personal yang hangat dan
  otentik, diperkaya dengan humor yang relevan. \textbf{Skor: 19/20
  (95\%).}
\item
  \textbf{UTS-2 (Songs for You):} Pesan kuat dengan interpretasi lagu
  yang sangat personal dan menyentuh. \textbf{Skor: 18/20 (90\%).}
\item
  \textbf{UTS-3 (My Stories for You):} Contoh penceritaan yang luar
  biasa, naratif, dan sangat inspiratif. \textbf{Skor: 20/20 (100\%).}
\item
  \textbf{UTS-4 (My SHAPE):} Analisis SHAPE mendalam, koheren, dan
  dirangkum dengan sangat baik dalam Piagam Diri. \textbf{Skor: 19/20
  (95\%).}
\end{itemize}

\subsubsection{1.2 Penilaian UTS-5 (Telaah Kritis terhadap Karya
Sendiri)}\label{penilaian-uts-5-telaah-kritis-terhadap-karya-sendiri}

Bagian ini adalah implementasi dari tugas UTS-5 yang sebenarnya:
melakukan telaah kritis terhadap sebuah karya. Saya memilih untuk
menganalisis \textbf{UTS-3 ``My Story For You''} sebagai objek telaah.

\begin{itemize}
\tightlist
\item
  \textbf{Pemahaman Konsep Interpersonal (5/5):} Cerita ``Pelajaran dari
  Kotak Kardus'' secara komprehensif mendemonstrasikan konsep
  \textbf{pemberdayaan (empowerment)} dan \textbf{mentoring}. Penulis
  tidak mengambil alih tugas, melainkan memfasilitasi adiknya untuk
  menemukan solusinya sendiri, menunjukkan pemahaman mendalam tentang
  komunikasi yang membangun kemandirian.
\item
  \textbf{Analisis Kritis Pesan (5/5):} Analisis saya tajam dalam
  mengidentifikasi bahwa kekuatan cerita ini bukan pada hasil akhir
  dioramanya, melainkan pada momen transformatif saat sang adik
  menyadari potensinya (``Kak, ternyata aku bisa, ya?''). Ini adalah
  pesan inti yang berhasil diungkap.
\item
  \textbf{Argumentasi (Logos) (5/5):} Argumen bahwa cerita ini adalah
  ilustrasi sempurna dari nilai-nilai di UTS-1 sangat logis dan
  meyakinkan. Terdapat benang merah yang jelas antara ``nilai kepedulian
  dari kakek-nenek'' dengan ``tindakan membantu adik'', yang membuat
  seluruh portofolio terasa koheren.
\item
  \textbf{Etos \& Empati (5/5):} Saat menulis ini, saya berusaha
  menempatkan diri sebagai pembaca, memastikan narasi mengalir dengan
  empati dan diakhiri dengan pesan yang beresonansi secara universal,
  menunjukkan etos yang berimbang.
\item
  \textbf{Rekomendasi Perbaikan (5/5):} Saya merekomendasikan penambahan
  pertanyaan reflektif di akhir cerita (``Momen kecil apa yang telah
  mengajarkan Anda pelajaran terbesar?'') sebagai saran yang sangat
  konkret dan aplikatif untuk meningkatkan keterlibatan pembaca.
\end{itemize}
...
\tightlist
\item
  \textbf{UTS-4 (Keterlibatan):} Meskipun sudah terstruktur, bagian
  SHAPE bisa dibuat lebih menarik secara visual, misalnya dengan
  menggunakan ikon atau infografis sederhana untuk merepresentasikan
  setiap elemen (Strengths, Heart, etc.).
\end{itemize}
...
\subsubsection{2.1 Penilaian untuk: {[}Isi Nama \& NIM Rekan
Anda{]}}\label{penilaian-untuk-isi-nama-nim-rekan-anda}

\begin{itemize}
\tightlist
\item
  \textbf{URL Portofolio:} {[}Salin URL portofolio rekan Anda di sini{]}
\item
  \textbf{Tinjauan Umum:} {[}Tulis beberapa kalimat tentang kesan umum
  Anda terhadap karya rekan Anda. Apa yang paling menonjol? Apa kekuatan
  utamanya?{]}
\item
  \textbf{Saran Konstruktif:} {[}Berikan satu saran spesifik yang bisa
  membantu rekan Anda mengembangkan karyanya lebih lanjut. Fokus pada
  aspek yang bisa ditingkatkan.{]}
\end{itemize}

\subsubsection{2.2 Penilaian untuk: {[}Isi Nama \& NIM Rekan
Anda{]}}\label{penilaian-untuk-isi-nama-nim-rekan-anda-1}
...
\subsubsection{2.3 Penilaian untuk: {[}Isi Nama \& NIM Rekan
Anda{]}}\label{penilaian-untuk-isi-nama-nim-rekan-anda-2}
